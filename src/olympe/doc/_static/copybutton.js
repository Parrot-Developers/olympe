...
const addCopyButtonToCodeCells = () => {
  ...
  const clipboard = new ClipboardJS('.copybtn', {
    text: (trigger) => {
        ...
        const htmlBlock = document.querySelectorAll('div.highlight pre'+query)[0]
        if (htmlBlock.parentElement.parentElement.classList.contains("highlight-console")) {
          text = ""
          htmlBlock.innerText.split('\n').forEach((line) => {
            if (line.match(/^.*\$/))
              text += line.replace(/^.*\$ ?/, '') + "\n"
            else if (line.match(/^ *In \[\d+\]:/))
              text += line.replace(/^ *In \[\d+\]: ?/, '') + "\n"
          })
          return text;
        }
        else {
